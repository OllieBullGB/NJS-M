...
#include "MLLoggingApp.h"

Define_Module(MLLoggingApp);

MLLoggingApp::MLLoggingApp() {
...
void MLLoggingApp::initialize(int stage)
{
    ...
    if (stage == INITSTAGE_LOCAL)
    {
        ...
        EV << "MLLoggingApp Successfully Init" << endl;
    }
}

void MLLoggingApp::handleMessageWhenUp(cMessage *msg)
{
...
double MLLoggingApp::calculateDistanceToCoord(Coord position)
{
...
void MLLoggingApp::addReport(std::vector<SNRReportElement> report)
{
    EV_INFO << "MLLoggingApp Received report of size: " << report.size() << " there are " << nodes.size() << " nodes registered" << endl;

    for (SNRReportElement& reportElement : report)
    {
        // add report to recordings
        MLNode* node = relevantNode(reportElement);
        if (node == nullptr)
        {
            // Create a new NJSLegacyNode directly and add it to the nodes vector
            MLNode newNode(reportElement.getNode(), reportElement.getPosition());
...
MLNode* MLLoggingApp::relevantNode(SNRReportElement reportElement)
{
    MLNode* ptr = nodes.data();

    for (MLNode* end = ptr + nodes.size(); ptr != end; ++ptr)
    {
...
Coord MLLoggingApp::localiseJammer()
{
...
Coord MLLoggingApp::calculateAveragePoint(const std::vector<Coord>& coords)
{
    double sumX = 0.0;
    double sumY = 0.0;
    double sumZ = 0.0;
...
double MLLoggingApp::calculateJammerLocalisationError(Coord& predictedPosition, const char* jammerName)
{
...
std::array<int, 4> MLLoggingApp::calculateJammingAccuracy(const char* jammerName)
{
    int tp = 0;
    int fp = 0;
    int tn = 0;
...
void MLLoggingApp::recordAndSaveMetricsAtTimestep()
{
    ...
    saveResultsToFile(jle, accuracy, ttfd, speed, "/home/veins/workspace.omnetpp/InetWirelessTutorial/simulations/NJS_ap/results/snrMLResults.txt");
}


void MLLoggingApp::saveResultsToFile(double jle, std::array<int, 4> accuracy, double ttfd, double speed, const std::string& filename)
{
