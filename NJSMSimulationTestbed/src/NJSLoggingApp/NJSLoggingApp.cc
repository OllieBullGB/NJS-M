...
void NJSLoggingApp::handleMessageWhenUp(cMessage *msg)
{
    if (msg == processReportsTimer)
    {
        ...
        for (NJSDetectionZone& existingDz : detectionZones)
        {
            ...
            std::vector<NJSState> rsuTimeline = rsuDz.timelines[rsuDz.getNodeTimelinePosition("RSU")];
...
void NJSLoggingApp::recordAndSaveMetricsAtTimestep()
{
    ...
    saveResultsToFile(jle, accuracy, ttfd, "/home/veins/workspace.omnetpp/InetWirelessTutorial/simulations/NJS_ap/results/results.txt");
}

void NJSLoggingApp::saveResultsToFile(double jle, std::array<int, 4> accuracy, double ttfd, const std::string& filename)
{
